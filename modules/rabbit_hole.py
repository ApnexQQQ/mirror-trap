...
def generate_fake_layer(attacker_ip: str, depth: int, context: str = "",
                        fingerprint: str = "", probe_count: int = 0,
                        attacker_type: str = "unknown", attacker_actions: list = None) -> dict:
    """
...
def generate_attacker_profile(sessions, fingerprint):
    if not DEEPSEEK_API_KEY or not sessions:
        return {}
    s = sessions[0] if sessions else {}
    prompt = (
        "Analyze this honeypot attacker and respond ONLY in JSON with these exact keys: "
        "threat_actor_type, sophistication, likely_motivation, likely_target, likely_country, "
        "campaign_stage, threat_score (0-100), recommended_action. "
        "Attacker: IP=" + str(s.get("ip","?")) +
        " type=" + str(s.get("attacker_type","?")) +
        " tool=" + str(s.get("likely_tool","?")) +
        " ports=" + str(s.get("ports","?")) +
        " skill=" + str(s.get("skill_level","?")) +
